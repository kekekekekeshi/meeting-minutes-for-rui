    ...
    async def process_transcript(self, text: str, model: str, model_name: str, chunk_size: int = 5000, overlap: int = 1000) -> Tuple[int, List[str]]:
        ...
        try:
            ...
            # --- END OPENAI SUPPORT ---
                ...
                api_key = await db.get_api_key("deepseek")
                if not api_key: raise ValueError("OPENAI_API_KEY environment variable not set")
                llm = OpenAIModel(model_name, api_key=api_key, base_url='https://api.deepseek.com/v1')
                logger.info(f"Using Deepseek model: {model_name}")
            elif model == "rui_deepseek":
                api_key = await db.get_api_key("rui_deepseek")
                if not api_key: raise ValueError("OPENAI_API_KEY environment variable not set")
                llm = OpenAIModel(model_name, api_key=api_key, base_url='请将接口地址配置到此：在transcript_processor.py的115行。')
                logger.info(f"Using Deepseek model: {model_name}")
            else:
            ...
            for i, chunk in enumerate(chunks):
                ...
                try:
                    ...
                    summary_result = await agent.run(
                        f"""给定以下会议记录块，根据所需的 JSON 结构提取相关信息。如果特定部分（例如“关键截止日期”）在此块中没有相关信息，则为其“块”返回一个空列表。确保输出仅为 JSON 数据。
